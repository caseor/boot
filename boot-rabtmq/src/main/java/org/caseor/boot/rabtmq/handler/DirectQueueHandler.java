...
public class DirectQueueHandler {
   ...
   */
  // @RabbitHandler
  public void directHandleAutoAck(MessageStruct messageStruct) {
    log.info(" 直接队列处理器, 自动ACK, 接受消息:{} ", JsonUtil.serialize(messageStruct));
  }
  ...
  public void directHandleManualAck(MessageStruct messageStruct, Message message, Channel channel) {
    ...
    try {
      log.info(" 直接队列处理器, 手动ACK, 接受消息:{} ", JsonUtil.serialize(messageStruct));
